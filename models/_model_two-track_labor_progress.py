...
_DPARAM = {
    ...
    'delta': {
        'func': lambda tau_delta=1, itself=0: -itself / tau_delta,
        'initial': 0.04,
        'eqtype': 'ode',
    },
    'N': {
    ...
    'w': {
        'func': lambda L=np.zeros(2), N=np.ones(2), itself=np.zeros(2), tau=1: (w0 * L / (N - L) - itself) / tau,
        'initial': np.array([50e3, 120e3]),
        'eqtype': 'pde',
    },
    'labor_density': {
        'func': del_t_labor_density,
        'initial': (2.5e6 * lognormal_distribution(0.5e6, kl_sigma, geom_kl_ratios)
                    + 2.7e6 * lognormal_distribution(6.3e6, kl_sigma, geom_kl_ratios)),
        'eqtype': 'pde',
    },
